<template>
  ...
  <div v-else-if="store" class="store-page">
    <Breadcrumb
      :items="[
        { label: $t('nav.home'), to: '/' },
        { label: $t('businesses.title'), to: '/stores' },
    ...
    <div v-if="activeTab === 'offers'">
      <BusinessOffersHero
        v-if="store.featuredOffer"
        :offer="store.featuredOffer"
        @claim="onClaimOffer"
      />
      ...
      <div v-if="store.offers && store.offers.length" class="row g-3">
        <div v-for="offer in store.offers" :key="offer.id" class="col-md-6 col-lg-4">
          <BusinessOfferCard :offer="offer" @claim="onClaimOffer" />
        </div>
      </div>
    ...
    <!-- منتجات -->
    <div v-else-if="activeTab === 'products'" class="row g-3">
      <div v-for="product in store.products" :key="product.id" class="col-6 col-md-4 col-lg-3">
        <ProductCard :product="product" />
      </div>
      <UiEmptyState
        v-if="!store.products || !store.products.length"
        :title="$t('businesses.noProducts')"
        :description="$t('businesses.noProductsDesc')"
      />
    </div>
...
const tabs = computed(() => {
  ...
  return [
    ...
    { key: 'products', label: t('businesses.tabs.products'), icon: 'bi bi-box-seam', count: (s.products && s.products.length) || 0 },
...
async function loadStore() {
  ...
  try {
    ...
    activeTab.value = (store.value.offersCount > 0) ? 'offers' : 'products'
  } catch (err) {
